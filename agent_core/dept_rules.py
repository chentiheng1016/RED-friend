...
DEPT_RULES: Dict[str, Dict[str, List[str]]] = {
    ...
    "會計": {
        ...
        "sender_patterns": ["bank.example", "fx-desk", "cht_ebpp", "cht.com.tw",
                            "mega-bank", "ctbcbank", "bank2.example", "esun.com.tw"],
    },
}
...
def classify_dept(sender: str, subject: str) -> Tuple[str, List[str]]:
    ...
    for dept, rules in DEPT_RULES.items():
        ...
        if any(sender_low == _norm(s) for s in senders):
            ...
            continue
        # sender substring pattern（case-insensitive；用來抓 bank.example / cht_ebpp 這類外部自動通知）
